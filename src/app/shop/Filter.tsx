...
const Filter = ({ totalProduct }: { totalProduct: number }) => {
  ...
  const { FilterData } = useContext(StoreData);
    ...
    <div className={`max-w-[1440px] mx-[auto] ${style.filter}`}>
      <div className={`flex w-full items-center justify-around ${style.smallScreenSize}`}>
        <div className={`flex items-center gap-[3vw] ${style.smallSize}`}>
          ...
          <div>
            <h1 className={`${style.showing}`}>Showing 1-{totalProduct} of {totalProduct} results</h1>
          </div>
        </div>

        <div className={`flex gap-[3vw] ${style.smallSize}`}>
          <div className={`flex items-center gap-4 ${style.smallSize}`}>
            <span className={`${style.showingData}`}>Show</span>
            <span className={`w-[48px] h-[48px] bg-white flex items-center justify-center ${style.showNumber}`}>{totalProduct}</span>
          </div>

          <div className={`flex gap-[3vw] items-center`}>
            <span className={`${style.showingData}`}>Sort by</span>
            <span className={`w-[100px] h-[48px] bg-white flex items-center justify-center ${style.showNumber}`}>Default</span>
          </div>
        </div>
      </div>
    </div>
