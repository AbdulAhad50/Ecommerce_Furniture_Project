...
const Checkout = () => {
    ...
    let  [items]:any[] = placedOrder
    console.log('/***/',items);
    // let {totalPrice, totalQuanity, totalName} = items

    console.log(items?.totalPrice);
    console.log(items?.totalQuanity);
    console.log(items?.totalName);


    function truncateLongWords(arr) {
        return arr?.map(word => {
          if (word.length > 10) {
            return word.slice(0, 10) + '...';  // Pehle 10 characters, phir 3 dots
          }
          return word;
        });
      }

      let data = truncateLongWords(items?.totalName);
            ...
            <div className={`w-[40%] flex flex-col gap-3 ${style.customerDetail}`}>
                  ...
                  <div className='mt-5'>
                        ...
                        <div className='flex flex-col justify-between '>
                            ...
                            {
                            data?.map((elem,i)=>{
                                console.log(`${items?.totalQuanity[i] * items?.singleProductPrice[i]}`)

                                return <div className='my-4'>
                                    <h3 className={`${style.placeDetailProductName}`}>{elem} <p className={`${style.placeDetailProductQuantity}`}>x {items?.totalQuanity[i]}</p></h3>
                                    <p className={`${style.placeDetailProductPrice}`}>{`$ ${items?.totalQuanity[i] * items?.singleProductPrice[i]}`}</p>
                                </div>
                            })
                        ...
                        <div className='flex justify-between items-center'>
                            ...
                            <p className={`${style.placeDetailProductSubTotalPrice}`}>{`$ ${items?.totalPrice}`}</p>
                        </div>
